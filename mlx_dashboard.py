...
def download_model(repo_id: str, root: Path) -> str:
    ...
    console.print("[bold cyan]Downloading " + repo_id + " to " + str(target_dir) + "...[/bold cyan]")
...
def run_model_setup_wizard(root: Path) -> None:
    if not sys.stdin.isatty():
        raise RuntimeError(
            "No local MLX models found. Run the dashboard in a terminal to execute the model setup wizard, or set PRIMARY_MODEL_PATH and FALLBACK_MODEL_PATH."
        )

    console.print("[bold cyan]◈ MLX MODEL SETUP ◈[/bold cyan]")
    console.print("No local MLX models were found.")
    console.print("[1] Download Qwen3.5-27B + Qwen3.5-9B (default)")
    console.print("[2] Enter custom Hugging Face model IDs")
    choice = console.input("Choice [1]: ").strip() or "1"

    if choice == "2":
        primary_repo = console.input("Primary Model ID: ").strip()
        fallback_repo = console.input("Fallback Model ID: ").strip()
        if not primary_repo or not fallback_repo:
            raise RuntimeError("A Hugging Face model ID is required for both Primary and Fallback.")
    elif choice == "1":
        primary_repo = DEFAULT_PRIMARY_MODEL_REPO
        fallback_repo = DEFAULT_FALLBACK_MODEL_REPO
    else:
        raise RuntimeError("Invalid choice in model setup.")
    ...
    try:
        ...
        raise RuntimeError("Model download failed: " + str(error)) from error

    os.environ["PRIMARY_MODEL_PATH"] = primary_path
    os.environ["FALLBACK_MODEL_PATH"] = fallback_path
    console.print("[bold green]Models downloaded. Starting router...[/bold green]")
...
def discover_model_paths():
    ...
    if not candidates:
        ...
        candidates = find_local_model_paths(root)
    if not candidates:
        raise RuntimeError("No MLX models found under " + str(root) + " after model setup.")
...
def load_saved_config():
    ...
    for name in (
        ...
        "REASONING_EFFORT_27B", "THINKING_BUDGET_27B", "ROUTE_ON_SWAP", "SWAP_LIMIT_27B", "AUTO_RESTART_27B",
    ):
        if name in saved:
...
def save_config():
    ...
    for name in (
        ...
        "PREFILL_STEP_SIZE", "PROMPT_CONCURRENCY", "DECODE_CONCURRENCY",
        "MAX_GENERATION_TOKENS_27B", "THINKING_ENABLED_27B",
        "REASONING_EFFORT_27B", "ROUTE_ON_SWAP", "SWAP_LIMIT_27B", "AUTO_RESTART_27B",
    ):
...
def dither_bar(percent: float, width: int = 40) -> Text:
    ...
    partial_blocks = "▏▎▍▌▋▊▉"
    partial_index = int(remainder * len(partial_blocks))
    if full_blocks < width:
        if partial_index:
            hue = 120 + int(60 * full_blocks / width)
            bar.append(partial_blocks[partial_index - 1], style=f"rgb(0,{hue},180)")
    background_start = full_blocks + (1 if partial_index else 0)
    for i in range(background_start, width):
        ...
        if target_port == PRIMARY_PORT:
            ...
            body = inject_chat_template_kwargs(
                body,
                {
                    ...
                    "thinking_budget": THINKING_BUDGET_27B,
                },
            )
        elif target_port == FALLBACK_PORT:
    ...
    def _forward(self, method: str, body: bytes = None):
        ...
        if self.command == "POST" and self.path == "/v1/chat/completions" and self.dashboard:
            ...
            with self.dashboard.lock:
                ...
                self.dashboard.process_start_time = time.time()
...
class MLXDashboard:
    ...
    def start_server(self):
        cmd = [sys.executable, "-u", "mlx_server_safe_wrapper.py"]
        env = os.environ.copy()
        env.update({
    ...
    def start_fallback_server(self):
        if not AUTO_START_FALLBACK and not LAZY_FALLBACK:
            return
        cmd = [sys.executable, "-u", "mlx_server_fallback_wrapper.py"]
        env = os.environ.copy()
    ...
    def configure(self, live):
        ...
        global REASONING_EFFORT_27B, THINKING_BUDGET_27B, MAX_GENERATION_TOKENS_27B
        global ROUTE_ON_SWAP, SWAP_LIMIT_27B, AUTO_RESTART_27B
        global FALLBACK_MEMORY_LIMIT
        live.stop()
        self.keyboard.stop()
        settings = [
            ("Primary Model Path", MODEL_PATH, str),
            ("Fallback Model Path", FALLBACK_MODEL_PATH, str),
            ("Fallback Metal Limit GB", FALLBACK_MEMORY_LIMIT / 1024 ** 3, float),
            ("Total Context", MAX_CONTEXT_TOKENS, int),
            ("Context Safety Margin", CONTEXT_SAFETY_MARGIN, int),
            ("AUTO: Primary Prompt Limit", TOKEN_LIMIT_27B, int),
            ("Prefill Step Size", PREFILL_STEP_SIZE, int),
            ("Prefill Concurrency", PROMPT_CONCURRENCY, int),
            ("Decode Concurrency", DECODE_CONCURRENCY, int),
            ("Primary Max Generation", MAX_GENERATION_TOKENS_27B, int),
            ("Primary Thinking", "ON" if THINKING_ENABLED_27B else "OFF", str),
            ("Primary Reasoning", REASONING_EFFORT_27B, str),
            ("Thinking Budget Tokens", THINKING_BUDGET_27B, int),
            ("SWAP Routing", "ON" if ROUTE_ON_SWAP else "OFF", str),
            ("SWAP Limit GB", SWAP_LIMIT_27B, float),
            ("Auto-Restart Primary", "ON" if AUTO_RESTART_27B else "OFF", str),
        ]
        values = [item[1] for item in settings]

        def render_config(selected):
            console.clear()
            table = Table(title="MLX CONFIGURATION", border_style="cyan", expand=True)
            ...
            console.print("\n[dim][↑/↓ or J/K] Select  [E/Enter] Edit  [S] Save  [Q/Esc] Cancel[/dim]")
        ...
        try:
            ...
            if saved:
                config_values = [
                    ...
                    "THINKING_ENABLED_27B", "REASONING_EFFORT_27B", "THINKING_BUDGET_27B", "ROUTE_ON_SWAP", "SWAP_LIMIT_27B",
                    "AUTO_RESTART_27B",
                ]
                for name, value in zip(config_values, values):
                    if name == "FALLBACK_MEMORY_LIMIT":
                        value = int(float(value) * 1024 ** 3)
                    elif name == "SWAP_LIMIT_27B":
                        value = float(value)
                    elif name in ("THINKING_ENABLED_27B", "ROUTE_ON_SWAP", "AUTO_RESTART_27B"):
                        value = str(value).lower() in ("on", "an", "j", "ja", "y", "yes", "1", "true")
                    globals()[name] = value
                save_config()
                console.clear()
                console.print("[bold green]Configuration saved.[/bold green]")
                restart = console.input("Restart server now? [Y/n]: ").strip().lower()
                if restart not in ("n", "no", "nein", "0"):
            ...
            console.print("\n[bold red]Invalid input or configuration canceled.[/bold red]")
    ...
    def _idle_indicator(self) -> Text:
        ...
        if self.is_processing:
            started = self.process_start_time or self.last_infer_time or time.time()
            elapsed = max(0, int(time.time() - started))
            return Text("⚡ PROCESSING (" + str(elapsed) + "s)", style="bold yellow")
    ...
    def _hotkey_bar(self) -> Text:
        ...
        hotkeys = [
            ...
            ("1", "Always Primary", "bold green"),
            ("2", "Always Fallback", "bold yellow"),
            ("X", "Fallback on/off", "bold red"),
            ("Z", "Primary on/off", "bold red"),
            ("C", "Clear", "bold yellow"),
            ("P", "Print", "bold green"),
            ("K", "Config", "bold magenta"),
